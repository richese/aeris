									...
									)
{
	...
	nn_init_structure->learning_constant_eta = learning_constant_eta;
	nn_init_structure->learning_constant_gamma = learning_constant_gamma;
...
CNeuralNetwork::CNeuralNetwork(struct sNeuralNetworkInitStructure nn_init_structure)
{
	...
	nn.learning_constant_eta = nn_init_structure.learning_constant_eta;
	nn.learning_constant_gamma = nn_init_structure.learning_constant_gamma;
	...
	nn.error_prev = (float**)malloc(nn.layers_count*sizeof(float*));
	
	nn.input = (float**)malloc(nn.layers_count*sizeof(float*));
	nn.input_ = (float**)malloc(nn.layers_count*sizeof(float*));
	nn.input_prev_ = (float**)malloc(nn.layers_count*sizeof(float*));
	...
	for (k = 0; k < nn.layers_count; k++)
	{
		...
		nn.error_prev[k] = (float*)malloc(nn.size_output[k]*sizeof(float));
		nn.input[k] = (float*)malloc(nn.size_input[k]*sizeof(float));
		nn.input_[k] = (float*)malloc(nn.size_input_[k]*sizeof(float));
		nn.input_prev_[k] = (float*)malloc(nn.size_input_[k]*sizeof(float));
		...
		for (j = 0; j < nn.size_output[k]; j++)
		{
			...
			for (i = 0; i < nn.size_input_[k]; i++)
				nn.w[k][j][i] = nn.weight_range*rnd()*0.1;

			nn.output[k][j] = 0.0;
			nn.error[k][j] = 0.0;
			nn.error_prev[k][j] = 0.0;
		}
...
CNeuralNetwork::~CNeuralNetwork()
{
	...
	{	
		...
		free(nn.error_prev[k]);
		nn.error_prev[k] = NULL;
		...
		free(nn.input_prev_[k]);
		nn.input_prev_[k] = NULL;
	}
...
void CNeuralNetwork::process(std::vector<float> input)
{
	...
	for (k = 0; k < nn.layers_count; k++)
	{
		...
		//allow taylor series
		if ( (nn.neuron_type == NEURON_TYPE_TAYLOR) || (nn.neuron_type == NEURON_TYPE_MIXED) )
			tmp_c = 1.0;

		//allow signals multiplication
		...
		for (j = 0; j < nn.size_input[k]; j++)
		{
			nn.input_prev_[k][ptr] = nn.input_[k][ptr];
			nn.input_[k][ptr] = tmp_a*nn.input[k][j]; 
			ptr++;
		}
		...
		for (i = j; i < nn.size_input[k]; i++)
			{
				nn.input_prev_[k][ptr] = nn.input_[k][ptr];
				nn.input_[k][ptr] = tmp_b*nn.input[k][j]*nn.input[k][i];
				ptr++;
			}
		...
		for (i = 0; i < nn.size_input[k]; i++)
			{
				nn.input_prev_[k][ptr] = nn.input_[k][ptr];
				nn.input_[k][ptr] = tmp_c*pow(nn.input[k][i], j);
				ptr++;
			}
...
void CNeuralNetwork::learn(std::vector<float> required_output)
{
	...
	float eta = nn.learning_constant_eta;
	float gamma = nn.learning_constant_gamma;

	float weight_range = nn.weight_range;

	for (i = 0; i < nn.size_output[nn.layers_count - 1]; i++)
	{
		nn.error_prev[nn.layers_count-1][i] = nn.error[nn.layers_count-1][i];
	...
	for (k_ = 0; k_ < nn.layers_count; k_++)
	{
		...
		for (j = 0; j < nn.size_output[k]; j++)
		{
			...
			{
				float dw = 0.0;
				dw+= eta*nn.input_[k][i]*nn.error[k][j];
				dw+= gamma*(nn.input_[k][i] - nn.input_prev_[k][i])*( nn.error[k][j] - nn.error_prev[k][j] );
